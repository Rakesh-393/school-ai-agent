...
class Branch(db.Model):
    ...
    code = db.Column(db.String(15), unique=True)          # "PHS", "PHER"
    locality = db.Column(db.String(80))                   # "Mankammathota"
    city = db.Column(db.String(60), default="Karimnagar")
    state = db.Column(db.String(60), default="Telangana")
    board = db.Column(db.String(40))                      # State (SSC) | CBSE | Cambridge (CIE)
    grades = db.Column(db.String(60))                     # "Pre-K to Grade 10"
    address = db.Column(db.String(240))
    phone = db.Column(db.String(60))
    email = db.Column(db.String(120))
    website = db.Column(db.String(120))
    principal = db.Column(db.String(120))
    established = db.Column(db.String(30))
    aliases = db.Column(db.String(300), default="")       # "explorica|jyothinagar|cambridge"
...
class ClassSection(db.Model):
    ...
    grade = db.Column(db.String(20), nullable=False)      # "7"
    section = db.Column(db.String(10), nullable=False)    # "B"
    class_teacher = db.Column(db.String(80))
...
class Student(db.Model):
    ...
    bus_route = db.Column(db.String(40))
...
class Subject(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(60), nullable=False, index=True)   # "Mathematics"
    code = db.Column(db.String(15), unique=True)                  # "MATH"
...
class Exam(db.Model):
    __tablename__ = "exams"
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(60), nullable=False)     # "Term 1"
    academic_year = db.Column(db.String(15), default="2025-26")
...
class FeeInvoice(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    student_id = db.Column(db.Integer, db.ForeignKey("students.id"), nullable=False)
    term = db.Column(db.String(30), nullable=False)      # "Term 1 2025-26"
    head = db.Column(db.String(40), default="Tuition")   # Tuition | Transport | Lab
...
class TimetableSlot(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    class_section_id = db.Column(db.Integer, db.ForeignKey("class_sections.id"), nullable=False)
    weekday = db.Column(db.String(10), nullable=False)   # Monday..Saturday
    period = db.Column(db.Integer, nullable=False)
    subject_id = db.Column(db.Integer, db.ForeignKey("subjects.id"))
    starts_at = db.Column(db.String(10))                 # "09:00"
...
class ChatLog(db.Model):
    ...
    role_context = db.Column(db.String(20))    # parent | teacher | admin
    question = db.Column(db.Text)
    answer = db.Column(db.Text)
    tools_used = db.Column(db.String(300))
